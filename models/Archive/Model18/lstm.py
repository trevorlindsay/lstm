...
class LSTM(object):
    ...
    def __init__(self, is_training, config):
        ...
        gru_cell = tf.nn.rnn_cell.GRUCell(num_units=size)

        if is_training:
            print 'Memory Cell: {}'.format(type(gru_cell))

        # Wrap the memory cell in a dropout layer (for outputs)
        if is_training and config.keep_prob < 1:
            gru_cell = tf.nn.rnn_cell.DropoutWrapper(cell=gru_cell, output_keep_prob=config.keep_prob)

        # Create the RNN with 'num_layers' layers
        stacked_cell = tf.nn.rnn_cell.MultiRNNCell([gru_cell] * config.num_layers)
        ...
        self.save_state = tf.Variable(tf.zeros([batch_size, size * config.num_layers]))
