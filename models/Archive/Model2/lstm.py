...
class LSTM(object):
    def __init__(self, is_training, config):
        ...
        if is_training:
            msg = 'Dense Activation: {}'.format(dense_activation)
            print msg
        ...
        if dense_activation:
            self._predictions = predictions = dense_activation(tf.matmul(output, dense_w) + dense_b)
        else:
        ...
        if is_training:
            ...
            msg = 'Optimizer: {}'.format(optimizer)
            print msg
...
def main(debug=False):
    ...
    # Do not buffer the output (NEEDED TO GET FULL LOG FILE)
