...
class DataSet(object):
    ...
    def prepBatches(self, num_steps):
        ...
        # Separate observations into input dataframes by timeindex
        features = [group for _, group in self.X.groupby(level=0)]

        # Determine how much padding is required for each dataframe (batch size will be length of largest group size)
        self.pads = pads = [max_group_size - group.shape[0] for group in features]

        # Pad the bottom of the dataframes with the mean
        features = [group.append([group.mean(level=0)] * pads[i]) if pads[i] != 0 else group for i, group in enumerate(features)]

        # Separate targets into target series by timeindex
        targets = [group for _, group in self.Y.groupby(level=0)]

        # Pad the series to the bottom with the mean
        targets = [group.append([group.mean(level=0)] * pads[i]) if pads[i] != 0 else group for i, group in enumerate(targets)]
    ...
    def __iter__(self):
        ...
        # features.shape = (timesteps, max_group_size, num_features)
        # targets.shape = (timesteps, max_group_size)
        ...
        # For when len(timesteps) % num_steps != 0, pad the matrices to the left with old values
        while epoch_size * num_steps < len(self.timesteps):
            inser_x = features[epoch_size * num_steps:]
            inser_y = targets[epoch_size * num_steps:]
        ...
        """
        # Old way of iterating over data
...
def load_data(debug=False):
    ...
    # Determine which timestep has the max number of instruments -- the number of instruments will be batch_size
