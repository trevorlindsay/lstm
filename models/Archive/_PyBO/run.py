...
def run_model(args, train_data, test_data, id1_train, id1_test, num_epochs, NUM_CORES, debug=False, final_run=False):
    ...
    with tf.Graph().as_default(), tf.Session(config=tf_config) as session:
        ...
        print 'Train r2: {}'.format(compute_r2(pd.Series(train_data.Y.values[:,0]), ytrain_pred))

        # Test predictions
        ytest_pred, _, _ = run_epoch(session, mtest, test_data, tf.no_op())
        print 'Test r2: {}'.format(compute_r2(pd.Series(test_data.Y.values[:,0]), ytest_pred))
    ...
    for step, (x, y) in enumerate(data):
        ...
        if not extra_steps:
            targets = [target[:, :-pad] if pad != 0 else target for pad, target in
                       zip(pads, np.split(preds, m.num_steps))]
        else:
            targets = []
            for i, (pad, target) in enumerate(zip(pads, np.split(preds, m.num_steps))):
                if i >= extra_steps:
                    if pad != 0:
