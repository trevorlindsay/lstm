...
class DataSet(object):
    ...
    def prepBatches(self, num_steps):
        ...
        # Separate observations into input matrices by timeindex
        features = [group for _, group in self.X.groupby(level=0)]

        # Determine how much padding is required for each matrix (batch size will be length of largest group size)
        self.pads = pads = [max_group_size - group.shape[0] for group in features]

        # Pad the matrices to the right with the mean
        features = np.asarray([np.pad(group, ((0, pads[i]), (0, 0)), 'mean') for i, group in enumerate(features)])

        # Separate targets into target vectors by timeindex
        targets = [group for _, group in self.Y.groupby(level=0)]

        # Pad the vectors to the right with the mean
        targets = np.asarray([np.pad(group, ((0, pads[i]), (0, 0)), 'mean') for i, group in enumerate(targets)])

        # Reshape to make it two dimensional (original shape with have unnecessary 3rd dimension of 1)
...
def getDataConfig(debug=False):
    ...
    with open(config['Data'], 'rb') as f:
        ...
        for i in range(len(header.split(','))):
            ...
            if col == 'id1':
                id1 = i
            if 'feature' in col or col == 'timeindex' or 'risk' in col or 'cat' in col or col == 'Category0':
