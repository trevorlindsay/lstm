...
class LSTM(object):
    ...
    def __init__(self, is_training, config):
        ...
        if is_training and config.keep_prob < 1:
            rnn_cell = tf.nn.rnn_cell.DropoutWrapper(cell=rnn_cell, output_keep_prob=config.keep_prob)

        # Create the RNN with 'num_layers' layers
        stacked_cell = tf.nn.rnn_cell.MultiRNNCell([rnn_cell] * config.num_layers)
